...
Shader "Custom/HoloTriangles" {
    Properties {
        _Color           ("Main Color",       Color)        = (0.2, 0.5, 1.0, 1.0)
    ...
    SubShader {
        ...
        Pass {
            ...
            float3 smallTrianglesColor(float3 pos) {
                ...
                c = ((c + float2(c.y, 0.0)*cos(a)) / zoom)
                    + float2(floor((c.x - c.y*cos(a)) / (zoom*4.0)) / 4.0, 0.0);

                float type = r(floor(c*4.0))*0.2
                           + r(floor(c*2.0))*0.3
                           + r(floor(c))*0.5;
                type += 0.3 * sin(_Time.y * 5.0 * type);

                float l = min(min(
                        1.0 - 2.0 * abs(frac((c.x - c.y)*4.0) - 0.5),
